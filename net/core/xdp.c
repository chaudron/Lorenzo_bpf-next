...
void xdp_return_num_frags_from_buff(struct xdp_buff *xdp, u16 num_frags)
{
	...
	sinfo = xdp_get_shared_info_from_buff(xdp);
	num_frags = min_t(u16, num_frags, sinfo->nr_frags);
	for (i = 1; i <= num_frags; i++) {
		skb_frag_t *frag = &sinfo->frags[sinfo->nr_frags - i];
		struct page *page = skb_frag_page(frag);

		sinfo->data_len -= skb_frag_size(frag);
		xdp->frame_length -= skb_frag_size(frag);
		__xdp_return(page_address(page), &xdp->rxq->mem, false, NULL);
	}

	sinfo->nr_frags -= num_frags;
	if (sinfo->nr_frags)
		xdp_buff_set_mb(xdp);
	else
