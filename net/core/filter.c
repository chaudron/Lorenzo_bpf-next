...
BPF_CALL_2(bpf_xdp_adjust_head, struct xdp_buff *, xdp, int, offset)
{
	...
	if (unlikely(xdp->data_hard_start + xdp->mb.headroom != xdp->data)) {
		pr_err("data needs reset running bpf_xdp_adjust_data(0)\n");
		return -EINVAL;
	}
...
BPF_CALL_2(bpf_xdp_adjust_tail, struct xdp_buff *, xdp, int, offset)
{
	...
	if (unlikely(xdp->data + xdp->mb.data_len != xdp->data_end)) {
		pr_err("data needs reset running bpf_xdp_adjust_data(0)\n");
		return -EINVAL;
	}
...
BPF_CALL_2(bpf_xdp_adjust_data, struct xdp_buff *, xdp, u32, offset)
{
	...
	u32 base_offset = xdp->mb.data_len;
	int i;

	if (!xdp_buff_is_mb(xdp) || offset > sinfo->xdp_frags_size)
		return -EINVAL;

	if (offset < xdp->mb.data_len) {
		/* linear area */
		xdp->data = xdp->data_hard_start + xdp->mb.headroom;
		xdp->data_end = xdp->data + xdp->mb.data_len;
		return 0;
	}
